    ...
    while ((ent = readdir(dir)) != NULL)
    {
        ...
        if (f)
        {
            ...
            while (fgets(line, sizeof(line), f))
            {
                ...
                {
                    if (strstr(line, TARGET_VID))
                        match_vid = true;
                    if (strstr(line, TARGET_PID))
                        match_pid = true;
                }
...
int main()
{
    ...
    while (1)
    {
        ...
        while (1)
        {
            ...
            if (read(raw_fd, buf, sizeof(buf)) <= 0)
                break;
            int dev_idx = -1;
            if (buf[0] == 0x30)
                dev_idx = 0; // Top Screen
            else if (buf[0] == 0x38)
                dev_idx = 1; // Bottom Screen
                ...
                for (int i = 0; i < MAX_SLOTS; i++)
                {
                    ...
                    else
                    {
                        if (slot_active[dev_idx][slot])
                        {
                            ...
                            if (DEBUG)
                                printf("Finger %i, CID %i, CC %i, Screen %i: Lifted\n", slot, contact_id, contact_count, dev_idx);
                            slot_active[dev_idx][slot] = false;
                        }
                    }
                }
